...
void cpmDir() {
    ...
    uint8_t *entry_p = NULL;
    entry_p = &entry;
    while (buffer[line * 32] != 0xe5) {
        int point = line * 32;
        // put the 32 bytes into the entry variable
        for (int i = point; i < point + 32; i++) {
            *entry_p = buffer[point + i];
            *(entry_p)++;
        }
        // check if character is legal before displaying it
        printf("%s", entry.name);
        printf(".");
